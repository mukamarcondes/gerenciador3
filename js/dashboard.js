document.addEventListener("DOMContentLoaded", async () => {
  ...
  function preencherSelect(select, valores, labelPadrao) {
    ...
    [...new Set(valores.map((v) => (v || "").trim()))]
      .sort((a, b) => a.localeCompare(b, "pt-BR"))
      .forEach((valor) => {
        const option = document.createElement("option");
        option.value = valor || "__nenhum__";
        option.textContent = valor || "Nenhum";
  ...
  function renderGraficoResponsavel(usados) {
    const responsaveis = {};
    usados.forEach((u) => {
      const chaveResponsavel = (u.responsavel || "").trim() || "Nenhum";
      responsaveis[chaveResponsavel] = (responsaveis[chaveResponsavel] || 0) + 1;
    });
  ...
  function aplicarFiltros() {
    ...
    const valorFiltroResponsavel = document.getElementById("filtroResponsavel")?.value || "";
    const filtroResponsavel = normalizarTexto(valorFiltroResponsavel);
    ...
    const usados = usadosCache.filter((item) => {
      ...
      if (filtroResponsavel) {
        const responsavelAtual = normalizarTexto(item.responsavel);
        if (valorFiltroResponsavel === "__nenhum__") {
          if (responsavelAtual) return false;
        } else if (responsavelAtual !== filtroResponsavel) return false;
      }
