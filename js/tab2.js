document.addEventListener("DOMContentLoaded", async () => {
  ...
  const itensPorPaginaUsados = document.getElementById("itensPorPaginaUsados");
  ...
  let paginaAtual = 1;
  const ITENS_POR_PAGINA_KEY = "usados_itens_por_pagina";
  const ITENS_POR_PAGINA_VALIDOS = [10, 25, 50];
  let itensPorPagina = Number(localStorage.getItem(ITENS_POR_PAGINA_KEY)) || 10;
  if (!ITENS_POR_PAGINA_VALIDOS.includes(itensPorPagina)) itensPorPagina = 10;
  ...
  const RESPONSAVEIS = window.AppUtils?.RESPONSAVEIS || ["Fabricio", "Gorete", "Pós-vendas", "Priscila"];
  ...
  function render(lista) {
    ...
    listaPaginada.forEach((n, i) => {
      ...
      tr.innerHTML = `
        ...
        <td>${n.cidade || "-"}</td>
        <td>${operadoraCell}</td>
        <td class="numero-copiavel" style="cursor:pointer;" title="${AppUtils.formatPhone(n.number)}">${AppUtils.formatPhone(n.number)}</td>
        <td>${n.dataAlteracao || "-"}</td>
        <td>
          <select class="respSelect" ${tipoUsuario === "user" ? "disabled" : ""}>
            <option value="">Nenhum</option>
            ${RESPONSAVEIS.map((responsavel) => `<option value="${responsavel}" ${n.responsavel === responsavel ? "selected" : ""}>${responsavel}</option>`).join("")}
          </select>
        </td>
        <td>
          <input class="empInput" value="${n.empresa || ""}" title="${AppUtils.escapeHtml(n.empresa || "")}" style="background:transparent;border:none;width:90%;" ${tipoUsuario === "user" ? "disabled" : ""}>
        </td>
        <td>
          <select class="status-select" ${statusDisabled}>
            ...
            ${tipoUsuario === "admin" ? "<option value=\"reativar_livres\">Reativar para Livres</option>" : ""}
          </select>
        </td>
      ...
      tr.querySelector(".status-select").addEventListener("change", async (e) => {
        ...
        if (tipoUsuario === "user" && (destino === "ativado" || destino === "reativar_livres")) {
          AppUtils.toast("Usuario comum nao pode reativar numeros.", "warning");
          render(listaAtual);
          return;
        }

        if (destino === "reativar_livres") {
          const confirmado = await AppUtils.confirmDialog({
            title: "Reativar numero",
            message: `Deseja mover ${AppUtils.formatPhone(n.number)} para a tela de livres?`,
  ...
  if (itensPorPaginaUsados) {
    itensPorPaginaUsados.value = String(itensPorPagina);
    itensPorPaginaUsados.addEventListener("change", () => {
      const novoValor = Number(itensPorPaginaUsados.value);
      if (!ITENS_POR_PAGINA_VALIDOS.includes(novoValor)) return;
      itensPorPagina = novoValor;
      localStorage.setItem(ITENS_POR_PAGINA_KEY, String(novoValor));
      paginaAtual = 1;
      render(listaAtual);
    });
  }
